...
from torch_geometric.nn import GINEConv, global_add_pool, BatchNorm, LayerNorm
...
class FFNN(nn.Module):
    def __init__(self, in_size, out_size, hidden_sizes=None, dropout=0.2):
        super().__init__()
        ...
        for h in self.hidS:
            ...
            layers.append(nn.LayerNorm(h))  # add batch norm to FFNN
...
class GNN(nn.Module):
    def __init__(self, node_features, edge_features, hidden_channels):
        ...
        self.conv1 = GINEConv(gine_mlp1, eps=0.0,
                              train_eps=True, edge_dim=hidden_channels)
        self.conv2 = GINEConv(gine_mlp2, eps=0.0,
                              train_eps=True, edge_dim=hidden_channels)
        self.conv3 = GINEConv(gine_mlp3, eps=0.0,
                              train_eps=True, edge_dim=hidden_channels)
...
class ModelTwo(nn.Module):
    def __init__(self, node_features, edge_features, hidden_channels, solv_features, hidden_sizes=None, solv_hidden_sizes=None):
        ...
        self.ffnn_solv = FFNN(
            solv_features, hidden_channels, solv_hidden_sizes)
        self.ffnn = FFNN(2*hidden_channels, 1, hidden_sizes)

    def forward(self, xm, mol_edge_index, mol_edge_attr, mol_batch, solv_morgan):
        readout_vector_mol = self.gnn_mol(
            xm, mol_edge_index, mol_edge_attr, mol_batch)
        readout_vector_sol = self.ffnn_solv(solv_morgan)

        final_readout = torch.cat(
            [readout_vector_mol, readout_vector_sol], dim=-1)
